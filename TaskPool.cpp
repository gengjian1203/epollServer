...
CTaskPool::CTaskPool(const int & poolMaxSize)
...
void* CTaskPool::CheckIdleTask(void * arg)
{
    CTaskPool * pool = (CTaskPool*)arg;
    while(1)
    {
        pool->LockIdle();
        pool->RemoveIdleTask();
...
bool CTaskPool::GetStop()
{
    return m_bStop;
}

void CTaskPool::CheckIdleWait()
{
...
int CTaskPool::AddTask(task_fun fun, void *arg)
{
    int iRet = 0;
    if(0 != fun)
    {
...
void CTaskPool::LockIdle()
{
    pthread_mutex_lock(&m_idleMutex);
}

void CTaskPool::UnlockIdle()
{
    pthread_mutex_unlock(&m_idleMutex);
}

Task *CTaskPool::GetIdleTask()
{
    LockIdle();
    Task * task = NULL;
    if(!m_idleList.empty())
...
void CTaskPool::SaveIdleTask(Task*task)
{
    if(NULL != task)
    {
        task->fun = 0;
