        ...
        const int &threadPoolPre)
{
    ...
    m_taskPool = new CTaskPool(tastPoolSize);
    if(NULL == m_taskPool)
    {
        return -2;
    }
...
void CThreadPoolManager::LockTask()
{
    pthread_mutex_lock(&m_mutex_task);
}

void CThreadPoolManager::UnlockTask()
{
    pthread_mutex_unlock(&m_mutex_task);
}

void* CThreadPoolManager::TaskThread(void* arg)
{
    CThreadPoolManager * manager = (CThreadPoolManager*)arg;
    while(1)
    {
...
CThreadPool * CThreadPoolManager::GetThreadPool()
{
    return m_threadPool;
}

CTaskPool * CThreadPoolManager::GetTaskPool()
{
    return m_taskPool;
}

int CThreadPoolManager::Run(task_fun fun,void* arg)
{
    if(0 == fun)
    {
        return 0;
...
bool CThreadPoolManager::GetStop()
{
    return m_bStop;
}

void CThreadPoolManager::TaskCondWait()
{
    struct timespec to;
